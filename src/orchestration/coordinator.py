...
class Coordinator:
    ...
    def _step_process_results(self, target_date: date, force_refresh: bool) -> Dict[str, Any]:
        ...
        self.results_processor.interaction_logger.log_agent_start(
            "ResultsProcessor", 
            f"Processing results for {target_date - timedelta(days=1)}"
        )
    ...
    def _step_president(self, candidate_picks: List[Dict[str, Any]], 
        ...
        self.report_generator.save_agent_report(
            ...
            metadata={
                ...
                "best_bets": len([p for p in approved_picks if p.get("best_bet", False)]),
                "total_units": president_response.get("daily_report_summary", {}).get("total_units", 0.0)
            }
        )
    ...
    def _step_finalize(self, review: CardReview, picks: List[Pick], candidate_picks: List[Dict[str, Any]], 
        ...
        # Generate and save betting card
        card_text = self.report_generator.generate_betting_card(
            picks, 
            target_date, 
            review.approved,
            modeler_output=predictions,
        ...
        card_path = self.report_generator.save_report_to_file(
            card_text, 
        ...
        if review.approved:
            ...
            logger.info(f"📋 Betting card saved to {card_path} ({len(best_bet_picks)} best bets, {len(picks) - len(best_bet_picks)} other picks)")
            ...
            logger.info(f"📋 Betting card saved to {card_path} ({len(picks)} picks analyzed, but no bets placed)")
