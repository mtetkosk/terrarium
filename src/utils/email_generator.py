...
from src.data.storage import Database, GameModel, PickModel, BetModel
...
<body>""")
        ...
        # 2. Yesterday's recap (using LLM if available)
        if yesterday_results and yesterday_games:
            recap = self._generate_yesterday_recap(yesterday, yesterday_results, yesterday_games)
            if recap:
                html_parts.append('<div class="section">')
                html_parts.append('<h2><span class="emoji">🏆</span> YESTERDAY\'S RECAP</h2>')
                # Convert plain text recap to HTML with proper formatting
                recap_html = self._format_recap_as_html(recap)
                html_parts.append(f'<div class="recap-content">{recap_html}</div>')
                html_parts.append('</div>')
            else:
                # Fallback to basic superlatives if LLM fails
                superlatives = self._generate_superlatives(yesterday_games, yesterday_results)
                if superlatives:
                    html_parts.append('<div class="section">')
                    html_parts.append('<h2><span class="emoji">🏆</span> YESTERDAY\'S SUPERLATIVES</h2>')
                    for category, description in superlatives.items():
                        html_parts.append(f'<div class="superlative"><strong>{category}:</strong> {description}</div>')
                    html_parts.append('</div>')
            ...
            if best_bets:
                ...
                for i, bet in enumerate(best_bets, 1):
                    if isinstance(bet, dict):
                        ...
                        html_parts.append('<div class="bet-item">')
                        bet_text = f"{i}. {matchup} - <span class='bet-selection'>{selection}</span>"
                        if odds:
                            bet_text += f" ({odds})"
                        html_parts.append(f'<div class="bet-selection">{bet_text}</div>')
                        
                        if rationale:
                            html_parts.append(f'<div class="bet-rationale">💡 {rationale}</div>')
                        html_parts.append('</div>')
                    else:
        ...
        if today_presidents_report:
            ...
            if other_picks:
                ...
                for i, pick in enumerate(other_picks, 1):
                    html_parts.append(f'<div class="bet-item"><div class="bet-selection">{i}. {pick}</div></div>')
                html_parts.append('</div>')
    ...
    def _generate_plain_text_email(
        ...
        if yesterday_results and yesterday_games:
            recap = self._generate_yesterday_recap(yesterday, yesterday_results, yesterday_games)
            if recap:
                email_lines.append("🏆 YESTERDAY'S RECAP")
                email_lines.append("-" * 60)
                email_lines.append(recap)
                email_lines.append("")
            else:
                # Fallback to basic superlatives if LLM fails
                superlatives = self._generate_superlatives(yesterday_games, yesterday_results)
                if superlatives:
                    email_lines.append("🏆 YESTERDAY'S SUPERLATIVES")
                    email_lines.append("-" * 60)
                    for category, description in superlatives.items():
                        email_lines.append(f"• {category}: {description}")
                    email_lines.append("")
            ...
            if best_bets:
                ...
                for i, bet in enumerate(best_bets, 1):
                    if isinstance(bet, dict):
                        ...
                        bet_line = f"{i}. {matchup} - {selection}"
                        if odds:
                            bet_line += f" ({odds})"
                        email_lines.append(bet_line)
    ...
    def _get_yesterday_results(self, target_date: date) -> Optional[Dict[str, Any]]:
        """
        Get yesterday's results from results processor
        
        Args:
            target_date: Today's date (results_processor.process() will get results for target_date - 1)
        """
        yesterday = target_date - timedelta(days=1)
        
        if not self.results_processor:
            # Try to read from file
            results_file = self.reports_dir / "results_processor" / f"daily_results_{yesterday.isoformat()}.txt"
            if results_file.exists():
                return self._parse_results_file(results_file)
            return None
        
        try:
            # results_processor.process(target_date) processes bets placed on target_date - 1
            # So to get results for games on yesterday, we pass target_date (today)
            return self.results_processor.process(target_date, force_refresh=False)
        except Exception as e:
    ...
    def _parse_results_file(self, results_file: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Total picks
            match = re.search(r'Total Picks: (\d+)', content)
            if match:
                stats['total_picks'] = int(match.group(1))
            
            # Settled bets (this is what we actually want to use)
            match = re.search(r'Settled Bets: (\d+)', content)
            if match:
                stats['settled_bets'] = int(match.group(1))
            
            # Wins/Losses (case-insensitive, handle various formats)
            match = re.search(r'Wins:\s*(\d+)', content, re.IGNORECASE)
            if match:
                stats['wins'] = int(match.group(1))
            else:
                stats['wins'] = 0
            
            match = re.search(r'Losses:\s*(\d+)', content, re.IGNORECASE)
            if match:
                stats['losses'] = int(match.group(1))
            else:
                stats['losses'] = 0
            
            match = re.search(r'Pushes:\s*(\d+)', content, re.IGNORECASE)
            if match:
                stats['pushes'] = int(match.group(1))
            else:
                stats['pushes'] = 0
            
            # Profit/Loss
            match = re.search(r'Profit/Loss \(Units\): ([+-]?\d+\.?\d*)', content)
            if match:
                stats['profit_loss_units'] = float(match.group(1))
            
            match = re.search(r'Profit/Loss \(Dollars\): \$([+-]?\d+\.?\d*)', content)
            if match:
                stats['profit_loss_dollars'] = float(match.group(1))
    ...
    def _get_today_presidents_report(self, target_date: date) -> Optional[str]:
        """Get today's presidents report content (kept for backwards compatibility)"""
        report_file = self.reports_dir / "president" / f"presidents_report_{target_date.isoformat()}.txt"
        if report_file.exists():
            try:
                with open(report_file, 'r') as f:
    ...
    def _format_yesterday_performance(self, results: Dict[str, Any]) -> str:
        ...
        return f"Record: {record} | Profit: {profit_str}"
